...
public class CardSeeder {
    ...
    public CardSeeder(@Autowired CardService cardService, @Autowired CardSetService cardSetService) {
    ...
    public void seedCards() throws IOException {
        ...
        // Create the card set
        CreateCardSetDto cardSet = new CreateCardSetDto("RoundOneCards", "", createdCards.stream().map(Card::getId).collect(Collectors.toCollection(ArrayList::new)));
        ...
        cardSet = new CreateCardSetDto("RoundTwoCards", "", createdCards.stream().map(Card::getId).collect(Collectors.toCollection(ArrayList::new)));
        cardSetService.createCardSet(cardSet);
    }
}
