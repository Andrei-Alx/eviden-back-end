...
class CardJsonReaderTest {
    ...
    void readCards() throws IOException {
        List<CreateCardDto> cards = new ArrayList<CreateCardDto>(
                List.of(new CreateCardDto(
                            new ArrayList<>(
                                    List.of(
                                            new Tag("testKey", "testValue")
                                    )
                            ),
                            new ArrayList<>(
                                    List.of(
                                            new Translation("testLanguage", "testText")
                                    )
                            )
                    )
                )
        ...
        assertEquals(cards.get(0).getTranslations().size(), readCards.get(0).getTranslations().size());
